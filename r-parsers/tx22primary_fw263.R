# This program will seek out and parse all files in a fixed-width-263 character format.
# Following are some of the goals for this parser:
#
        ...
        if (party != "" & grepl(".txt$",f)){
            ...
            if (nc == 263){
                ...
                for (i in 1:NROW(xx)){
                    ...
                    mm <- str_match(xx$office[i], "Us Rep (\\d+) Us Dist (\\d+)") # El Paso County
                    if(!is.na(mm[1,1])){
                        xx$district[i] <- mm[1,2]
                        xx$office[i] <- "U.S. House"
                        next
